 ...
 * `ClaudelanceCore.sol`. Useful for UI / agent prompts that want to
 * reason about limits before making a chain call. The contract is still
 * the source of truth; redeploy = update this file too.
 */
export const CUSD_DECIMALS = 18 as const;

/** Protocol fee in basis points (200 = 2%). */
export const PROTOCOL_FEE_BPS = 200 as const;
export const BPS_DENOMINATOR = 10_000 as const;

/** Maximum claimers per bounty. */
...
/** Minimum bounty amount in cUSD wei (0.5 cUSD). */
export const MIN_BOUNTY_WEI = 500_000_000_000_000_000n; // 0.5e18
...
/** Bounty tier guidance (UI hints, not enforced on chain). */
